...
mod vectors {
    ...
    // This should be accepted by verify(), but rejected by verify_strict() and safe_verify().
    #[test]
    fn repudiation() {
...
/// tests the full security of safe_verify namely the test cases from [SSR:CGN20]
fn safe_verify_extensive_security() {
    ...
    for i in 0..data.len() {
        ...
        let b = verifying_key.safe_verify::<sha2::Sha512>(message, &signature);
...
mod integrations {
    ...
    fn sign_verify() {
        ...
        assert!(
            verifying_key
                .safe_verify::<sha2::Sha512>(good, &good_sig)
                .is_ok(),
            "Safe verification of a valid signature failed!"
        );
        assert!(
        ...
        assert!(
            verifying_key
                .safe_verify::<sha2::Sha512>(good, &bad_sig)
                .is_err(),
            "Safe verification of a signature on a different message passed!"
        );
        assert!(
        ...
        assert!(
            verifying_key
                .safe_verify::<sha2::Sha512>(bad, &good_sig)
                .is_err(),
            "Safe verification of a signature on a different message passed!"
        );
    }
