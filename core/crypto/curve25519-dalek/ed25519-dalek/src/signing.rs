...
impl SigningKey {
    ...
    /// safe verification with extra checks from the NIST note and the `Taming the many EdDSAs paper` [SSR:CGN20]
    /// Best is to use this function as it prevents specific attacks and as it verifies with including cofactor clearing
    #[allow(non_snake_case)]
    pub fn safe_verify(&self, message: &[u8], signature: &Signature) -> Result<(), SignatureError> {
        self.verifying_key
            .safe_verify::<sha2::Sha512>(message, signature)
    }
