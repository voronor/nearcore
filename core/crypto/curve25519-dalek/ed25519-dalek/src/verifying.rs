...
impl VerifyingKey {
    ...
    /// The following verification algorithm follows the NIST IR 8214B ipd specification
    /// including the checks shed light on in [SSR:CGN20] ``Taming the many EdDSAs''
    /// such as, rejecting non-canonic scalar s, rejecting low-order public key, and
    /// reject non-canonical EC points (e.g. public key and commitment R).
    /// It verifies using the batch-compatible formula  8(R + hA - sB) = 0
    /// This method is slightly more expensive (but more coherent with batch verification)
    /// as it multiplies by the cofactor
    #[allow(non_snake_case)]
    pub fn safe_verify<CtxDigest>(
        &self,
        message: &[u8],
        signature: &ed25519::Signature,
    ) -> Result<(), SignatureError>
        ...
        CtxDigest: Digest<OutputSize = U64>,
    {
        ...
        if signature.s >= curve25519_dalek::constants::BASEPOINT_ORDER {
            return Err(InternalError::Verify.into());
        };

        // check canonicity of R and A
        if !signature.R.is_canonical_y() || !self.compressed.is_canonical_y() {
            return Err(InternalError::Verify.into());
        }
        // start calculating 8(R + hA - sB) = 0
        // compute h = H(R,PK,m)
        let h = Self::compute_challenge::<CtxDigest>(None, &signature.R, &self.compressed, message);

        // this check slightly deviates from CGN as it prevents low order commitments R
        // however, it is following the NIST note on Ed25519 mentioning to reject low order R and vk
        // https://nvlpubs.nist.gov/nistpubs/ir/2022/NIST.IR.8214B.ipd.pdf
        // this check is computed after calculating h to decompress once R
        ...
        // s < L thus - s mod L = L-s mod L = L-s
        let mod_s = curve25519_dalek::constants::BASEPOINT_ORDER - signature.s;

        // calculate R + hA - sB
        let tmp_point =
            EdwardsPoint::vartime_double_scalar_mul_basepoint(&h, &self.point, &mod_s).compress();
        let tmp_point = tmp_point
            .decompress()
            .expect("Problem in decompressing R + hA - sB")
            + signature_R;

        // 8(R + hA - sB)
        if tmp_point.is_small_order() {
            Ok(())
        } else {
            Err(InternalError::Verify.into())
        }
